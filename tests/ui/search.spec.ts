...
test.describe('Trang tìm kiếm @real', () => {
  test('SEARCH-001: link về trang chủ hiển thị khi chưa có kết quả tìm kiếm', async ({
    searchPage,
  }) => {
  ...
  test('SEARCH-002: tìm kiếm jacket trả về sản phẩm phù hợp', async ({ searchPage }) => {
    await searchPage.goTo();
    await searchPage.expectLoaded();
  ...
  test('SEARCH-003: từ khóa không tồn tại hiển thị trạng thái không có kết quả', async ({
    searchPage,
  }) => {
    await searchPage.goTo();
    await searchPage.expectLoaded();
  ...
  test('SEARCH-004: tìm kiếm từ header cập nhật query trên URL', async ({ homePage, page }) => {
    await homePage.goTo();
  ...
  test('SEARCH-005: tìm kiếm không phân biệt chữ hoa chữ thường', async ({ searchPage }) => {
    await searchPage.goTo();
    await searchPage.expectLoaded();
  ...
  test('SEARCH-VAL-002: tìm kiếm ký tự đặc biệt không trả về toàn bộ products', async ({
    ...
    const visibleProducts = await searchPage.visibleResultCount(
